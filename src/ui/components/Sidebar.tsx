...
import { codeToFlagEmoji, getCountryCodeByName, capitalizeWords } from '../utils/countryUtils';
...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const handleConnect = async () => {
    ...
    setConnecting(true);
    const connectPromise = window.electron.connectToCountry(selectedCountry.name);
    ...
    toast.promise(connectPromise, {
      ...
      error: (error) => {
        ...
        return `Failed to connect: ${error instanceof Error ? error.message : 'Unknown error'}`;
      },
    });
    ...
    <div className="sidebar w-[280px] sm:w-[320px] min-w-[240px] sm:min-w-[260px] max-w-[300px] sm:max-w-[340px] flex flex-col gap-3 sm:gap-6">
      ...
      <div className="bg-[#181A20] border border-blue-500 shadow-xl p-4 sm:p-6 min-h-[480px] sm:min-h-[520px] relative">
        ...
        {connected ? (
          <div className="flex flex-col items-center justify-center py-8 gap-3 text-center h-full min-h-[480px] sm:min-h-[520px]">
            ...
            <span className="text-green-400 text-sm font-medium">Connected to VPN</span>
          ...
          <>
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-8 gap-3 text-center">
                <div className="text-orange-400 text-sm font-medium">Failed to load, will try again</div>
                <div className="text-gray-400 text-xs max-w-[200px]">
                  This is expected, retrying automatically...
                </div>
              </div>
            ) : filteredCountries.length === 0 ? (
              <div className="text-center text-gray-400 py-8">No countries available.</div>
            ) : (
              <ul className="max-h-80 sm:max-h-96 overflow-y-auto custom-scrollbar">
                {filteredCountries.map((c) => (
                  <li
                    key={c.name}
                    className={`flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-2 rounded cursor-pointer mb-1 transition-colors text-sm sm:text-base ${selectedCountry?.name === c.name ? 'bg-[#232F4B] text-blue-400' : 'hover:bg-[#232733]'}`}
      ...
      {selectedCountry && !connected && !loading && (
        <div className="bg-[#181A20] border border-blue-500 shadow-xl p-4 sm:p-6 relative">
          ...
          <div className="absolute -bottom-1 -left-1 w-2 h-2 bg-blue-500 rounded-full"></div>
          <div className="absolute -bottom-1 -right-1 w-2 h-2 bg-blue-500 rounded-full"></div>
          <div className="mb-2 text-xs sm:text-sm text-gray-300 font-semibold">Selected Country</div>
          <div className="flex items-center gap-2 mb-4">
            <span className="text-xl sm:text-2xl">{selectedCountry.flag}</span>
            <span className="font-bold text-base sm:text-lg">{capitalizeWords(selectedCountry.name)}</span>
          </div>
          <select
